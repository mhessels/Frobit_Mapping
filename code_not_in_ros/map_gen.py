...
class FrobomindMap:
    ...
    def genMap(self): 
        ...
        for local_map_number_x in range(0,int(self.x_size)):
            for local_map_number_y in range(0,int(self.y_size)):
                filename = "maps/local_map_" + str(local_map_number_x) + "_" \
    ...
    def getLocalMap(self,x_position,y_position,array):
        filename_to_open = "maps/local_map_" + str(x_position) + "_" + str(y_position) + self.fileExtension
        try:
    ...
    def visualize_now(self,center_x,center_y):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                try:
                    self.getLocalMapIdent(center_x+i,center_y+j,Matrix) # Takes from Ident at the moment for debugging purposes.
                except IOError: # If the file is not found, just pass, because we dont need that map for visualizing
                    print("LOL")
                    pass 
